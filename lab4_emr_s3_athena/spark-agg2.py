...
# Load datasets
transactions = spark.read.csv(f"s3://{BUCKET_NAME}/rental_vehicles/rental_transactions.csv", header=True, inferSchema=True)
locations = spark.read.csv(f"s3://{BUCKET_NAME}/rental_vehicles/locations.csv", header=True, inferSchema=True)
vehicles = spark.read.csv(f"s3://{BUCKET_NAME}/rental_vehicles/vehicles.csv", header=True, inferSchema=True)
...
# Write results to S3
# location_performance.write.parquet(f"s3://{BUCKET_NAME}/rental_vehicles/output/location_performance_metrics", mode="overwrite")
# vehicle_performance.write.parquet(f"s3://{BUCKET_NAME}/rental_vehicles/output/vehicle_performance_metrics", mode="overwrite")
