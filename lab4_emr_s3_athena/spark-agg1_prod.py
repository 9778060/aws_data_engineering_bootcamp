...
users = spark.read.csv(f"s3://{BUCKET_NAME}/rental_vehicles/users.csv", header=True, inferSchema=True)
...
# Write results to S3
transaction_metrics.write.parquet(f"s3://{BUCKET_NAME}/rental_vehicles/output/transaction_metrics", mode="overwrite")
user_metrics.write.parquet(f"s3://{BUCKET_NAME}/rental_vehicles/output/user_metrics", mode="overwrite")
