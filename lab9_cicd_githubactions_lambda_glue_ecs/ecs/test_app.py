...
def s3(aws_credentials):
    with mock_aws():
        ...
        s3.put_object(Bucket=BUCKET_NAME, Key='ecommerce-data/new/orders/test.csv', Body=csv_content)
        yield s3
...
def test_list_files(s3):
    files = app.list_files(BUCKET_NAME, 'ecommerce-data/new/orders')
    assert files == ['ecommerce-data/new/orders/test.csv'], "File list is incorrect"

def test_read_from_s3(s3):
    df = app.read_from_s3(BUCKET_NAME, 'ecommerce-data/new/orders/test.csv')
