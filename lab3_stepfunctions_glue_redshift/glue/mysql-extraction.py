...
DYNAMO_DB_TABLE = "incremental_load_configurations"
...
credentials = get_rds_credentials(DB_SECRET_NAME, REGION_NAME)
...
config_table = dynamodb.Table(DYNAMO_DB_TABLE)
...
def main():
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if load_type == 'incremental' and incr_column:
                if last_extracted_value:
                    sql += f" WHERE {incr_column} > '{last_extracted_value}'"
