...
try:
    ...
    schema = StructType([
        ...
        StructField("precission", StringType(), True),
    ...
    data_frame_with_timestamp = data_frame.withColumn(
        "timestamp", 
        unix_timestamp(concat(lit(f"{CURRENT_DATE} "), col("hour")), "yyyy-MM-dd HH:mm:ss").cast("timestamp")
    ).withColumn(
        "partition_hour", 
    ...
    s3_path = f"s3://{BUCKET_NAME}/aggregations/"
    s3_path_checkpoint = f"s3://{BUCKET_NAME}/checkpoints/"
