...
from pyspark.sql.functions import col, to_date, count, sum as _sum, avg, approx_count_distinct, expr, rank, desc, input_file_name, row_number, concat_ws
...
user_streams_df = (spark.read.csv(user_streams_path + "*", header=True, inferSchema=True).withColumn("source_file", input_file_name()))
...
song_kpis_df = user_streams_df.groupBy("track_id", "report_date", "source_file").agg(
