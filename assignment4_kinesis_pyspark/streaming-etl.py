...
songs_df = spark.read.schema(song_schema).csv(f"s3://{BUCKET_NAME}/songs.csv")
...
raw_stream_df = glueContext.create_data_frame.from_options(
    ...
    connection_options={
        "streamARN": f"arn:aws:kinesis:{REGION}:{ACCOUNT_NUMBER}:stream/user_song_streams",
        "classification": "json",
...
query = metrics_df.writeStream \
    .outputMode("append") \
    .format("parquet") \
    .option("path", f"s3://{BUCKET_NAME}/music_metrics/") \
    .option("checkpointLocation", f"s3://{BUCKET_NAME}/checkpoints/music_metrics/") \
    .start()
